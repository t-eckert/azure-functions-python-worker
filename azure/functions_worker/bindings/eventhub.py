...
class EventHubConverter(meta.InConverter, meta.OutConverter, binding="eventHub"):
    @classmethod
    ...
    def check_output_type_annotation(cls, pytype) -> bool:
        return issubclass(pytype, (str, bytes)) or (
            issubclass(pytype, typing.List) and issubclass(pytype.__args__[0], str)
        )
...
class EventHubTriggerConverter(
    EventHubConverter, binding="eventHubTrigger", trigger=True
):
    @classmethod
    def from_proto(
        cls, data: protos.TypedData, *, pytype: typing.Optional[type], trigger_metadata
