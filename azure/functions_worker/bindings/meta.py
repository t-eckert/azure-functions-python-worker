...
class _ConverterMeta(abc.ABCMeta):
    ...
    def __new__(
        mcls, name, bases, dct, *, binding: typing.Optional[str], trigger: bool = False
    ):
        ...
        if check_in_typesig is not None and not getattr(
            check_in_typesig, "__isabstractmethod__", False
        ):
        ...
        if check_out_typesig is not None and not getattr(
            check_out_typesig, "__isabstractmethod__", False
        ):
...
class _BaseConverter(metaclass=_ConverterMeta, binding=None):
    ...
    def _decode_typed_data(
        cls,
        data: typing.Optional[protos.TypedData],
        *,
    ...
    def _decode_trigger_metadata_field(
        cls,
        trigger_metadata: typing.Mapping[str, protos.TypedData],
        field: str,
        *,
        python_type: typing.Union[type, typing.Tuple[type, ...]],
    ) -> typing.Any:
    ...
    def _parse_datetime_metadata(
        cls, trigger_metadata: typing.Mapping[str, protos.TypedData], field: str
    ...
    @classmethod
    def _parse_datetime(cls, datetime_str: str) -> datetime.datetime:
    ...
    @classmethod
    def _parse_timedelta(cls, timedelta_str: str) -> datetime.timedelta:
...
def check_input_type_annotation(
    binding: str, pytype: type, datatype: protos.BindingInfo.DataType
) -> bool:
    try:
        checker = _ConverterMeta._check_in_typeann[binding]
    except KeyError:
        raise TypeError(
            f"{binding!r} input binding does not have " f"a corresponding Python type"
        ) from None
...
def check_output_type_annotation(binding: str, pytype: type) -> bool:
    try:
        ...
        raise TypeError(
            f"{binding!r} output binding does not have " f"a corresponding Python type"
        ) from None
    ...
    *,
    ...
    try:
        try:
            ...
            return converter(val, pytype=pytype, trigger_metadata=trigger_metadata)
