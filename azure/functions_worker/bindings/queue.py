...
class QueueMessageInConverter(meta.InConverter, binding="queueTrigger", trigger=True):
    @classmethod
    ...
    def from_proto(
        ...
        if data_type == "string":
            ...
            raise NotImplementedError(f"unsupported queue payload type: {data_type}")

        if trigger_metadata is None:
            raise NotImplementedError(f"missing trigger metadata for queue input")
...
class QueueMessageOutConverter(meta.OutConverter, binding="queue"):
    ...
    def check_output_type_annotation(cls, pytype: type) -> bool:
        ...
        return meta.is_iterable_type_annotation(pytype, valid_types) or (
    ...
    def to_proto(
        ...
        if isinstance(obj, str):
            ...
            return protos.TypedData(
                json=json.dumps({"id": obj.id, "body": obj.get_body().decode("utf-8")})
            )
            ...
            for item in obj:
                if isinstance(item, str):
                    ...
                    msgs.append(
                        {"id": item.id, "body": item.get_body().decode("utf-8")}
                    )
                else:
            ...
            return protos.TypedData(json=json.dumps(msgs))
