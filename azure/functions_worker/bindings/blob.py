...
class BlobConverter(meta.InConverter, meta.OutConverter, binding="blob"):
    @classmethod
    def check_input_type_annotation(
        cls, pytype: type, datatype: protos.BindingInfo.DataType
    ) -> bool:
        if (
            datatype is protos.BindingInfo.undefined
    ...
    def check_output_type_annotation(cls, pytype: type) -> bool:
        return issubclass(
            pytype,
            (str, bytes, bytearray, azf_abc.InputStream)
...
class BlobTriggerConverter(BlobConverter, binding="blobTrigger", trigger=True):
    pass
