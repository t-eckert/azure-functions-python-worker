    ...
    def add_function(
        ...
        for name, desc in metadata.bindings.items():
            if desc.direction == protos.BindingInfo.inout:
                raise FunctionLoadError(
                    func_name, f'"inout" bindings are not supported'
                )

            if name == "$return":
                if desc.direction != protos.BindingInfo.out:
                    raise FunctionLoadError(
                        func_name, f'"$return" binding must have direction set to "out"'
                    )
                ...
                if not bindings.is_binding(return_binding_name):
                    raise FunctionLoadError(
                        func_name, f'unknown type for $return binding: "{desc.type}"'
                    )
        ...
        if "context" in params and "context" not in bound_params:
            ...
            if "context" in annotations:
                ...
                if not isinstance(ctx_anno, type) or not issubclass(
                    ctx_anno, azf.Context
                ):
                    raise FunctionLoadError(
                        func_name,
        ...
        for param in params.values():
            ...
            is_param_out = (
                param_has_anno
                and (
                    typing_inspect.is_generic_type(param_anno)
                    and typing_inspect.get_origin(param_anno) == azf.Out
                )
                or param_anno == azf.Out
            )
            ...
            if not bindings.is_binding(param_bind_type):
                raise FunctionLoadError(
                    func_name, f'unknown type for {param.name} binding: "{desc.type}"'
                )
        ...
        if return_binding_name is not None and "return" in annotations:
            ...
            if (
                ...
                and typing_inspect.get_origin(return_anno) == azf.Out
            ):
                raise FunctionLoadError(
                    func_name, f"return annotation should not be azure.functions.Out"
                )
