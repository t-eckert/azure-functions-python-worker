...
DEFAULT_WEBHOST_DLL_PATH = (
    PROJECT_ROOT / "build" / "webhost" / "Microsoft.Azure.WebJobs.Script.WebHost.dll"
)
    ...
    async def load_function(self, name):
        ...
        for b in func.desc["bindings"]:
            ...
            bindings[b["name"]] = protos.BindingInfo(
                type=b["type"], direction=direction
            )
...
def start_webhost(*, script_dir=None, stdout=None):
    ...
    for n in range(10):
        try:
            r = requests.get(f"{addr}/api/ping", params={"code": "testFunctionKey"})
